...
def add_product(selection_id, product_name, product_price, product_brand):
    conn = sqlite3.connect(database_name)
    cursor = conn.cursor()
    
    cursor.execute('INSERT INTO Products (SelectionID, ProductName, ProductPrice, ProductBrand) VALUES (?, ?, ?, ?)',
                   (selection_id, product_name, product_price, product_brand))
...
def insert_data(number_of_products, product_list):
    ...
    for product in product_list:
        product_name, product_price, product_brand = product
        add_product(selection_id, product_name, product_price, product_brand)
